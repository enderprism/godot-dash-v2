...
use godot::classes::{CheckBox, Control, Engine, HBoxContainer, IHBoxContainer, Label};
use godot::prelude::*;
...
struct BoolProperty {
    ...
    #[init(val = OnReady::manual())]
    input: OnReady<Gd<CheckBox>>,
    base: Base<HBoxContainer>,
}

#[godot_api]
impl IHBoxContainer for BoolProperty {
    fn ready(&mut self) {
        let mut label = Label::new_alloc();
        let mut spacer = Control::new_alloc();
        let mut input = CheckBox::new_alloc();

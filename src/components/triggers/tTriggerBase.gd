...
@export var _touch_trigger: bool:
	set(value):
		...
		# _spawned_trigger = false
		_set_hitbox_shape(value, false)

@export var _spawned_trigger: bool:
	set(value):
		_spawned_trigger = value
		# _touch_trigger = false
		_set_hitbox_shape(false, value)
...
func _set_hitbox_shape(touch_trigger: bool, spawned_trigger: bool) -> void:
	if _hitbox != null:
		if touch_trigger:
			_hitbox.shape = RectangleShape2D.new()
			_hitbox.shape.size = Vector2.ONE * 128.0
		elif spawned_trigger:
			_hitbox.shape = RectangleShape2D.new()
...
func _ready() -> void:
	...
	_set_hitbox_shape(_touch_trigger, _spawned_trigger)
	add_child(_hitbox)
